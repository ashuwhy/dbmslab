...
export default function CourseDetailPage() {
  ...
  const fetchContent = useCallback(async () => {
    const res = await fetchWithAuth(
      `${API}/instructor/courses/${courseId}/content-items`,
    );
  ...
  const fetchTopics = useCallback(async () => {
    const res = await fetchWithAuth(
      `${API}/instructor/courses/${courseId}/topics`,
    );
  ...
  const handleDeleteContent = async (contentId: number) => {
    try {
      const res = await fetchWithAuth(
        `${API}/instructor/courses/${courseId}/content-items/${contentId}`,
        {
          method: "DELETE",
        },
      );
  ...
  const handleRemoveTopic = async (topicId: number) => {
    try {
      const res = await fetchWithAuth(
        `${API}/instructor/courses/${courseId}/topics/${topicId}`,
        {
          method: "DELETE",
        },
      );
  ...
  const handleCopyEmails = () => {
    ...
    setMessage({
      text: `${students.filter((s) => s.email).length} emails copied to clipboard!`,
      type: "success",
    });
  };
  ...
  const getContentIcon = (type: string) => {
    const iconClass = "w-8 h-8 text-zinc-400 shrink-0";
    switch (type) {
      case "video":
        return <HugeiconsIcon icon={CameraVideoIcon} className={iconClass} />;
      case "book":
        return <HugeiconsIcon icon={BookOpen01Icon} className={iconClass} />;
      case "notes":
        return <HugeiconsIcon icon={Note01Icon} className={iconClass} />;
      default:
  ...
  const getScoreBadge = (score: number | null) => {
    if (score === null)
      return <span className="text-zinc-600 text-sm italic">Pending</span>;
    if (score >= 80)
      return (
    ...
    if (score >= 60)
      return (
        <Badge className="bg-sky-900/50 text-sky-400 border-sky-800">
          {score}%
        </Badge>
      );
    if (score >= 40)
          ...
          <Card className="bg-zinc-900/50 border-zinc-800">
            ...
            <CardContent>
              <form onSubmit={handleAddContent} className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="content-title" className="text-zinc-400">
                    Title
                  </Label>
                  <Input
                    id="content-title"
                ...
                <div className="space-y-2">
                  <Label htmlFor="content-url" className="text-zinc-400">
                    URL
                  </Label>
                  <Input
                    id="content-url"
                ...
                <div className="space-y-2">
                  <Label htmlFor="content-type" className="text-zinc-400">
                    Type
                  </Label>
                  <select
                    id="content-type"
          ...
          <Card className="bg-zinc-900/50 border-zinc-800">
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="topic-select" className="text-zinc-400">
                ...
                <TableBody>
                  {students.map((student) => (
                    ...
                    >
                      ...
                      <TableCell>
                        {getScoreBadge(student.evaluation_score)}
                      </TableCell>
                      <TableCell className="text-right">
                        <Button
                          ...
                          onClick={() => {
                            ...
                            setGradeValue(
                              student.evaluation_score?.toString() || "",
                            );
                    ...
                    {Object.entries(analytics.distribution).map(
                      ([bucket, count]) => {
                        const maxCount = Math.max(
                          ...Object.values(analytics.distribution),
                          1,
                        );
                        ...
                        return (
                          <div key={bucket} className="flex items-center gap-3">
                            ...
                            <span className="text-xs text-zinc-300 w-6 font-mono">
                              {count}
                            </span>
                          </div>
                        );
                    ...
                    <div className="flex gap-4 text-xs">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-zinc-400">
                          Pass ({analytics.pass_rate}%)
                        </span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <div className="w-2.5 h-2.5 bg-red-500" />
                        <span className="text-zinc-400">
                          Fail ({(100 - analytics.pass_rate).toFixed(1)}%)
                        </span>
                      </div>
                    </div>
                  </div>
        ...
        <DialogContent>
          ...
          {gradingStudent && (
            <div className="space-y-4 py-2">
              ...
              <div className="space-y-2">
                <Label htmlFor="grade-input" className="text-zinc-400">
                  New Score (0–100)
                </Label>
                <Input
                  id="grade-input"
